...
- 若用户本会话已选 / 现在表示要**逐步确认模式** → `Read references/run-modes.md` 模式一，
  从断点起**每个写操作步骤前都停下问用户，一次只推进一步**，不要套用「默认自动续跑到完成点」。
- 若用户选**自动驾驶模式** → 按下面的续跑默认连续推进。
- 若用户只说「继续」且**未指定过模式** → 先问一句「每步确认还是一次性跑完？」再决定（除非用户明显赶时间/已说过别问）。
...
- **用户已经明确要求继续 / 恢复 / 从这里开始**
  - 不先停在进度展示或模式确认
  - 直接依据 `next_step` 进入对应 playbook 并推进
  - 默认把这次 continuation 当成一次自动续跑，直到当前请求对应的合理完成点
...
- 先找当前首个缺失的全局步骤
- 默认只把这个首个缺失全局步骤推进到完成点
- 达到该完成点后立刻停止
- 不在同一轮里继续自动推进下一个全局步骤
- 不自动继续到 `POST /projects/{project}/episodes/plan`、批量肖像或逐集执行，除非用户明确要求整段自动跑完
...
- 若用户说的是“从这里开始 / 从断点继续 / 继续做到下一步”，
  默认把**当前首个缺失步骤完成**当作这次 continuation 的合理完成点
- 达到该完成点后立即停止继续探测、停止额外轮询，直接汇报结果
