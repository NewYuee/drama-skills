...
| 方法 | 路径 | 用途 |
|------|------|------|
| `POST` | `/projects/{project}/episodes/{ep}/sketches/generate` | 草图 `{"style":"...","model":"nanobanana","grid_index":0,"sketch_location_grouping":true}` [ASYNC]。后端按 beat 数自动拆成 N 张 grid（2x4 起步，按容量 8/6/4/2/1 降级；18 beat→`2x4+2x4+1x2` 共 3 张）。**调用方必须对每个 `grid_index` 顺序调用**，每次等 SSE 完成；首次调用的 SSE result 里返回 `total_grids`。详见 `pipeline-details.md` Step 12。 |
