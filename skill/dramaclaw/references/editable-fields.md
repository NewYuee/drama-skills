...
`video_backend` 选择规则：
- `huimeng_seedance-1.0-pro-fast`：默认值；用户没有明确指定其它后端时，整集批量和单 beat 重做都用它。
- `huimeng_seedance-1.5-pro`：只在用户明确指定 1.5 Pro / 有声 1.5 / Huimeng 1.5 时传，不作为默认值。
- 已知限制：`feature/huimeng-video-provider` 当前整集批量接口只支持“选择一个 `video_backend`”，不支持同一次批量里按 beat 自动分流后端；不要把整集拆成多个单 beat 请求来模拟批量内分流。
